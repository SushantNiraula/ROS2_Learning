...
class MoveRobotClient(Node):
    ...
    def send_goal(self, position, velocity):
        self.move_robot_client_.wait_for_server() ## waiting for server to come up

        goal= MoveRobot.Goal()
        goal.position=position
        goal.velocity=velocity
        self.get_logger().info("Sending the goal. with position :" + str(position) +" and velocity: "+ str(velocity))
        self.move_robot_client_.send_goal_async(goal, feedback_callback=self.goal_feedback_callback).\
    ...
    def goal_result_callback(self, future):
        ...
        if status== GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().info("Succedded")
    ...
    def goal_feedback_callback(self, feedback_msg):
        positon=feedback_msg.feedback.current_position
        self.get_logger().info("Got feedback Position:  "+ str(positon))
